...
```text
登录页 -> auth store -> auth API 层 -> /api/login -> Redis session -> httpOnly cookie -> /api/me
```
...
stores/auth.ts
  只负责登录状态，例如 user、isLoggedIn、fetchMe、logout 后的状态清理。
...
也就是说，store 可以提供 `login()` 这个动作，但接口细节应该下沉到 API/service 层：
...
当前代码已经把 `stores/auth.ts` 里的接口请求抽到 `utils/api/auth.ts`，并改成 `httpOnly cookie` 模式。前端 store 不读写 token，只维护当前用户状态。
...
中间件不直接读取 token。页面访问时会通过 auth store 调用 `/api/me`，由服务端根据 `httpOnly cookie + Redis session` 判断是否登录：
...
管理登录状态，只维护当前用户、登录状态、获取当前用户和退出后的状态清理。

```text
utils/api/auth.ts
```

封装登录相关接口请求，例如 `loginApi`、`fetchMeApi`、`logoutApi`。store 不直接写死 `$fetch('/api/login')`。
