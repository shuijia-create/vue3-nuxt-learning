...
- `pages/work-orders/index.vue`：工单列表页面，通过 `useWorkOrders()` 读取数据库工单。
- `pages/work-orders/[id].vue`：工单详情页面，通过动态路由 id 查询数据库工单和处理记录。
...
菜单数据不是写死在前端文件里，而是登录成功后调用 getInfo，或刷新页面时由 getInfo 返回：
...
页面手动访问也要过权限校验，但不是每次跳转都请求接口。登录成功后会请求 `/api/me`，刷新页面时也会重新请求 `/api/me`，把当前用户拥有的页面路由权限返回给前端：
...
现在工单模块已经从 mock 数组升级到数据库读写。为了适合学习，项目没有拆成独立前后端，而是在 Nuxt 单项目里保持一条固定链路：

```text
pages/work-orders/*.vue
  页面：只处理筛选、弹窗、按钮、表格和提示
  ↓
composables/use-work-orders.ts
  页面业务：列表查询、新增工单、详情查询、状态流转、AI 草稿
  ↓
utils/api/work-orders.ts
  API client：统一保存前端请求地址、参数和返回类型
  ↓
server/api/work-orders/*
  后端入口：读取 query/body/route param，拿当前用户，调用 service
  ↓
server/services/work-orders.ts
  业务逻辑：参数校验、状态流转规则、编号生成、通知和日志编排
  ↓
server/utils/prisma.ts + prisma/schema.prisma
  数据库：读写 work_orders、notifications、operation_logs
```

这条链路的学习重点是职责边界：

- `store` 只保存状态，例如当前用户、通知列表，不直接发请求。
- `page` 不直接拼接口地址，页面只关心“我要查询/提交/刷新”。
- `utils/api` 统一放 HTTP 请求，后面接口地址变了只改这里。
- `server/api` 不写复杂业务，只做请求入口。
- `server/services` 放真正业务逻辑，数据库读写统一通过 Prisma。
- Redis 只保存登录 session，工单、通知、日志这些主业务数据进入 MySQL。

更完整的解释见：

```text
docs/fullstack-work-order-flow.md
```
...
> 这一节保留最早的 mock 学习过程，方便回看 Nuxt `server/api` 是怎么一步步搭起来的。当前实际工单链路已经升级为 `page -> composable -> API client -> server api -> service -> Prisma DB`。
...
当前项目已经把登录链路按企业项目常见方式拆开：

```text
登录页 -> auth composable -> auth API 层 -> 统一响应解包 -> /api/login 返回 token -> /api/me 返回 getInfo
```

不要在 store 里直接写死接口请求，例如：
...
utils/api/auth.ts 或 services/auth.ts
  负责封装接口请求，例如 loginApi、fetchMeApi、logoutApi，并把 { code, message, data } 统一解包成业务数据。
...
登录接口的真实业务数据放在 `data` 里，例如 `data.token`。前端 API client 会统一解包，所以 composable 里仍然可以直接写 `const { token } = await loginApi(form)`。

登录接口只负责返回 token 和建立登录态。用户信息、菜单路由、按钮权限统一由 `/api/me` 返回，这就是后台项目里常说的 getInfo；登录成功后第一次 getInfo 会带 `Authorization: Bearer <token>`。

当前 Nuxt SSR 学习项目为了刷新页面时能在服务端恢复登录态，仍然会把 token 写入 `httpOnly cookie`。前端 store 不发请求，只维护当前用户和权限快照。
...
中间件不直接读取 token。页面访问时会通过 `useAuth().fetchCurrentUser()` 调用 `/api/me`，由服务端根据 `httpOnly cookie + Redis session` 判断是否登录：

```ts
const currentUser = await authActions.fetchCurrentUser()
```

如果没有当前用户，并且访问的不是 `/login`，就跳转到登录页：
...
```ts
setCookie(event, 'nuxt-admin-token', token, {
  path: '/',
  httpOnly: true,
  sameSite: 'lax',
  maxAge: 60 * 60 * 24
...
然后返回本次登录的 `token`。用户信息、菜单路由和按钮权限不从登录接口返回，而是登录成功后继续调用 `GET /api/me` 获取。
...
它会通过 `useAuth().fetchCurrentUser()` 请求：
...
管理登录状态，只维护当前用户和登录状态，只暴露 `setUser`、`clearUser` 这类状态修改方法。

```text
composables/use-auth.ts
```

封装登录、退出、拉取当前用户这些认证用例。它会调用 `utils/api/auth.ts`，再把结果写入 `stores/auth.ts`。

```text
utils/api/auth.ts
```

封装登录相关接口请求，例如 `loginApi`、`fetchMeApi`、`logoutApi`。store 和页面都不直接写死 `$fetch('/api/login')`。
...
8. 最后看 `composables/use-auth.ts` 和 `stores/auth.ts`，理解认证用例如何连接页面、接口和 Pinia 状态。
