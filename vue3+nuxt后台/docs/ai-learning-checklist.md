...
- [x] 增加工单处理记录
- [x] 增加操作日志
- [x] 完整工单流程支持受理、处理反馈、确认关闭和退回处理
- [x] 工单按类型自动流向处理部门，并按提交人、处理人、部门负责人控制可见范围
...
- [x] 整理 mock 企业制度文档
- [x] 创建知识库数据结构
- [x] 创建 `POST /api/ai/knowledge-answer`
- [x] 前端提问后返回答案
- [x] 返回引用来源
- [x] 明确提示“AI 回答不能替代制度原文”
...
- 2026-06-10：修正后台列表页高度布局，把数据页改成视口内固定三段结构，表格内容区内部滚动、分页固定在底部，并压缩权限树表格行高；通过 `npm run typecheck`、`npm run build` 验证。
- 2026-06-10：统一后台 UI 主题色、按钮、表格、状态标签和通知面板样式；工作台改为运营概览布局，工单列表补齐“查询区 / 表格区 / 底部分页”的三段式结构，并通过 `npm run typecheck`、`npm run build` 验证。
